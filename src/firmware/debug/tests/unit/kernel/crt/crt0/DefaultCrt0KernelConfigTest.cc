...
namespace smeg::tests::unit::kernel::crt::crt0
{
	...
	suite<> defaultCrt0KernelConfigTest("DefaultCrt0KernelConfig Test Suite", [](auto &unit)
	{
		...
		{
			expect(std::tuple_size_v<typename DefaultCrt0KernelConfig<1, 0, DummyMemorySection, DummyDriverConfigs>::Tasks>, equal_to(1));
			expect(std::tuple_size_v<typename DefaultCrt0KernelConfig<2, 0, DummyMemorySection, DummyDriverConfigs>::Tasks>, equal_to(2));
			expect(std::tuple_size_v<typename DefaultCrt0KernelConfig<10, 0, DummyMemorySection, DummyDriverConfigs>::Tasks>, equal_to(10));
		});

		unit.test("Tasks_getStackNumberOfSlots_expectEachIsSameAsTemplateArgument", []()
		{
			expect(allStacksNumberOfSlots(typename DefaultCrt0KernelConfig<1, 123, DummyMemorySection, DummyDriverConfigs>::Tasks{}), array(123));
			expect(allStacksNumberOfSlots(typename DefaultCrt0KernelConfig<2, 456, DummyMemorySection, DummyDriverConfigs>::Tasks{}), array(456, 456));
			expect(allStacksNumberOfSlots(typename DefaultCrt0KernelConfig<3, 789, DummyMemorySection, DummyDriverConfigs>::Tasks{}), array(789, 789, 789));
		});
		...
		unit.test("Tasks_getStackMemorySections_expectEachIsPlacedInMemorySectionPassedAsTemplateArgument", []()
		{
			expect(
				std::same_as<
					AllStacksMemorySections<typename DefaultCrt0KernelConfig<1, 0, StubMemorySection, DummyDriverConfigs>::Tasks>,
					std::tuple<StubMemorySection>>,
				equal_to(true));

			expect(
				std::same_as<
					AllStacksMemorySections<typename DefaultCrt0KernelConfig<3, 0, StubMemorySection, DummyDriverConfigs>::Tasks>,
					std::tuple<
						StubMemorySection,
						StubMemorySection,
						StubMemorySection>>,
		...
		unit.test("Drivers_get_expectSameTypeAsTemplateArgument", []()
		{
			expect(
				std::same_as<
					typename DefaultCrt0KernelConfig<18, 0, DummyMemorySection, StubDriverConfigs>::Drivers,
					StubDriverConfigs>,
				equal_to(true));
		});
	});
